...
app.post('/retrieveInfo', express.json(), async (req, res) => {
  let { wallet, page, channel_id } = req.body;
  try {
    ...
    if (containsENSDomain) {
	   const ensQuery = JSON.stringify({
		  "query": "query NFTsByENS($ensName:String!){ethereum{walletByENS(ensName:$ensName){address}}}",
		  "variables": {
			"ensName": wallet
		  }
		});
      ...
      response = await axios.post('https://api.quicknode.com/graphql', ensQuery, ensConfig);
      wallet = response.data.data.ethereum.walletByENS.address;
    }
    ...
    if (wallet !== null) {
      ...
      response = await axios.post(QUICKNODE_RPC_URL, rpcQuery, rpcConfig);
    } else {
      embedResponses = [
        {
    ...
    if (!embedResponses) {
      embedResponses=response.data.result.assets.map((datax) => {
        ...
        const name = collection.collectionName? (collection.collectionName.length > 256 ? collection.collectionName.slice(0, 253) : collection.collectionName) : "No name";
